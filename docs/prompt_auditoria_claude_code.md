# Prompt de Auditoria de Segurança — Claude Code (v1.0)

> **Uso:** colar como *system prompt* no Claude Code (`CLAUDE.md` do repositório, custom command, slash command, ou comando de sessão de auditoria dedicada). Compatível com o template `validacao_codigos_seguranca.md` v1.0.
> **Autor:** Carlos Resende — Head of IT, 24/04/2026.

---

## 🎯 Copie a partir daqui (prompt pronto)
...
### Opção A — Como `CLAUDE.md` do repositório
1. Salvar o bloco delimitado entre ```` ``` ```` como `.claude/CLAUDE.md` ou `CLAUDE.md` na raiz do repositório.
2. Claude Code carrega automaticamente em toda sessão aberta naquele repositório.
3. Para forçar reuso explícito, invocar: `Use o prompt de CLAUDE.md e execute audit:pr 123`.

### Opção B — Como slash command customizado
1. Criar `.claude/commands/security-audit.md` com o conteúdo.
2. Invocar via `/security-audit audit:pr 123` no Claude Code.

### Opção C — Como subagente dedicado
1. Criar em `.claude/agents/security-auditor.md`.
2. Definir `description: "Agente de auditoria de segurança — 4 camadas (C1–C4)"`.
3. Invocar com `Task(subagent_type: "security-auditor", prompt: "audit:pr 123")`.

### Opção D — Integração no CI (GitHub Actions)
Gatilho em PR abre sessão Claude Code headless, executa `audit:pr ${{ github.event.pull_request.number }}`, captura saída JSON e publica comentário no PR + gate de *status check* bloqueante se `verdict == "BLOCKED"`.
...
| Trimestre | Ação |
|---|---|
| Q2/2026 | Publicar v1.0 em repositório interno de governança; piloto em 2 repositórios críticos. |
| Q3/2026 | Incorporar métricas: *findings*/PR, taxa de falso-positivo, *time-to-remediate*. |
| Q4/2026 | Ajustar prompt com base nos dados; publicar v1.1. |
| Q1/2027 | Revisão semestral contra novas versões OWASP/ASVS/LLM Top 10. |

---

*Prompt redigido por Carlos Resende — Head of IT, em 24/04/2026. Alinhado ao template `validacao_codigos_seguranca.md` v1.0.*
